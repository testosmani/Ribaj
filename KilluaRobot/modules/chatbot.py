    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Killua Chatbot disable by {}.".format(
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Killua Chatbot enable by {}.".format(
                    mention_html(user.id, user.first_name)
                ),
                parse_mode=ParseMode.HTML,
            )
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        kukiurl = requests.get(
            "https://www.kukiapi.xyz/api/apikey=KUKIg76Fg4EIo/Natsunagi/@apisuserbot/message="
            + Message
        )
...
def list_all_chats(update: Update, context: CallbackContext):
    ...
    text = "<b>KilluaRobot Enabled Chats</b>\n"
    for chat in chats:
        try:
...
__help__ = """
Chatbot utilizes the Osmani's api which allows Kuki to talk and provide a more interactive group chat experience.

*Admins only Commands*:
➢ `/Chatbot`*:* Shows chatbot control panel
  
*Powered by @ribajosmani*
"""
